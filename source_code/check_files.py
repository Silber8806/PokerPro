...
files_to_check=[]
for f in glob.glob("data/poker_balances*"):
    numbers = ''.join([c for c in f if c in string.digits])
    lines = sum([1 for line in open(f,'r')])
    files_to_check.append((numbers,lines))

files_to_check = sorted(files_to_check,key=lambda k: int(k[0]))
